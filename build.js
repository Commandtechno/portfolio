...
const html = fs.readFileSync("html/index.html", "utf8").replace(/{.+}/g, file => {
  file = file.slice(1, -1);
...
const css = fs.readFileSync("css/index.css", "utf8").replace(/\/\* {.+} \*\//g, file => {
  file = file.slice(4, -4);
...
const js = fs.readFileSync("js/index.js", "utf8").replace(/\/\/ {.+}/g, file => {
  file = file.slice(4, -1);
